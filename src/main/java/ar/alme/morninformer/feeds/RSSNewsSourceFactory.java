...
public class RSSNewsSourceFactory {
	...
	public RSSNewsSource createLaNacionUltimas()
		...
		if (url != null) {
			RSSNewsSource newsSource = new RSSNewsSource(feedDescription, url,
					new LNFeedReader());
			return newsSource;
		} else
	...
	public RSSNewsSource createTwitter(String screenName) {
		...
		if (baseUrl != null) {
			...
			NewsSourceDescription feedDescriptionWithProperty = feedDescription
					.withProperty("screenName", screenName);
			String url = this.buildTwitterTimelineURL(baseUrl, screenName);
			RSSNewsSource newsSource = new RSSNewsSource(
					feedDescriptionWithProperty, url, new TwitterFeedReader());
			return newsSource;
		} else
			// should not be null, that would mean you didn't tell the factory
			// to learn about twitter, so, do it! (developer problem)
			throw new RuntimeException(new UnknownNewsSourceException(
					feedDescription.toString()));
	}
