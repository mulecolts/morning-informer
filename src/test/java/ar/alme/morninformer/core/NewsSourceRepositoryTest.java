...
public class NewsSourceRepositoryTest {
	...
	public void setUp() {
		...
		RSSNewsSourceFactory.instance().learn(NewsSourceDescription.RSS_CLARIN,
				URL_CLARIN);
		RSSNewsSourceFactory.instance().learn(
				NewsSourceDescription.RSS_LA_NACION_ULTIMAS,
				URL_LA_NACION_ULTIMAS);
		RSSNewsSourceFactory.instance().learn(NewsSourceDescription.TWITTER,
				URL_TWITTER);
	}

	@Test
	public void testRSSs() throws UnknownNewsSourceException {
		NewsSource newsSource = repository
				.get(NewsSourceDescription.RSS_CLARIN);
		NewsSource newsSource1 = repository
				.get(NewsSourceDescription.RSS_CLARIN);

		assertSame(newsSource, newsSource1);

		NewsSource newsSourceLN = repository
				.get(NewsSourceDescription.RSS_LA_NACION_ULTIMAS);
		...
		NewsSource newsSourceLN2 = repository
				.get(NewsSourceDescription.RSS_LA_NACION_ULTIMAS);
	...
	public void testTwitter() throws UnknownNewsSourceException {
		NewsSource newsSource = repository.get(NewsSourceDescription.TWITTER
				.withProperty("screenName", "mnsaldivar"));
		NewsSource newsSource1 = repository.get(NewsSourceDescription.TWITTER
				.withProperty("screenName", "mnsaldivar"));

		assertSame(newsSource, newsSource1);

		NewsSource newsSource2 = repository.get(NewsSourceDescription.TWITTER
				.withProperty("screenName", "rodrigouroz"));

		assertNotSame(newsSource1, newsSource2);
		assertNotSame(newsSource, newsSource2);

		NewsSource newsSource3 = repository.get(NewsSourceDescription.TWITTER
				.withProperty("screenName", "mnsaldivar").withProperty("foo",
						"bar"));
